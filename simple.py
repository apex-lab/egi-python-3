...
class _Format(object):
    ...
    def pack(self, key, *args):
        ...
        newargs = list(map(lambda x : bytes(x, "utf-8") if isinstance(x, str) else x, args))
        result = struct.pack(fmt, bytes(key, "utf-8"), *newargs)
        # if type(*args) == 'string':
        #     result = struct.pack(fmt, bytes(key, "utf-8"), bytes(*args, "utf-8"))
        #     return result
        # else:
        #     result = struct.pack(fmt, bytes(key, "utf-8"), *args)
        #     return result
        return result
...
def pstring(s):
    ...
    ps = struct.pack(fmt, bytes(s,'utf-8'))
...
class _DataFormat(object):
    ...
    def _pack_dict(self, table, pad = False):
        ...
        if not pad:
            # "try" ...
            list(map(EgiError.check_type, keys))
            list(map(EgiError.check_len, keys))
    ...
    def _make_event_header(self, size_of_the_rest, timestamp, duration, keycode):
        ...
        result_str = struct.pack("=sH2L4s", b'D', total_length, timestamp, duration, bytes(keycode, 'utf-8'))
...
class Netstation(object):
    ...
    def GetServerResponse(self, b_raise=True):
        ...
        """
        code = self._socket.read(1).decode("utf-8")
    ...
    def SendSimpleEvent(self, markercode, timestamp=None):
        ...
        self._socket.write(bytes(data_string, "utf-8"))
